//********************** Bibliothèques à inclure en plus **********************//
...
const byte rxPin = 9;
const byte txPin = 8;                                                     //Nous utilisons pas cette broche mais c'est pour l'objet de gestion de com

//Création objet - Gestion communication
...
void setup() {
  ...
  hc05Serial.begin(9600);                                                 // Communication série module bluetooth
...
void loop() {
  ...
  if(ReserveAlerte(ConversionTtoVol())== 1){                                
     couleur = "*LR255G0B0*";                                             // Définir couleur voyant - Rouge
     Serial.println("Le volume est " + String(ConversionTtoVol()) + " mL. Nous sommes en réserve.");
  }
  else{
    couleur = "*LR0G255B0*";                                              // Définir couleur voyant - Vert
    Serial.println("Le volume est " + String(ConversionTtoVol()) + " mL.");
  }
  
  hc05Serial.println(couleur);
  Serial.println(couleur);
  
  // Envoyer la valeur à la jauge
  hc05Serial.println("*V" + String(round(ConversionTtoVol()*100/VolumeMaxContenant)) + "*");                                                 
  Serial.println("*V" + String(round(ConversionTtoVol()*100/VolumeMaxContenant)) + "*");
  // Envoyer la valeur au graphe                                          // **TODO Potentiel ** - est-ce que cela va garder les valeurs précédentes ??
  hc05Serial.println("*G" + String(ConversionTtoVol()) + "*");
  Serial.println("*G" + String(ConversionTtoVol()) + "*");
